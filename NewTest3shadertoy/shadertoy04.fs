...
vec3 skyColor = vec3(.3,.45,0.5);
vec3 sunColor = vec3(0.9,.5,.9);   // Energy 

mat3 m = mat3( .00,  0.80,  .90,
              -0.80,  0.46, -0.48,
              -0.60, -.68,  0.44);
...
vec3 computeEnlighting( in vec3 cameraPos, in vec3 cameraDir, in vec3 lightDir ) {
    ...
    cameraDir += perturb3(cameraDir,.06,3.);
...
void main()
{
    ...
    //camera
    vec3 cameraPos = vec3(1.0,.6,1.5);      
    vec3 cameraTarget = vec3(0.0, 0.0,0.0);
    vec3 ww = normalize( cameraPos - cameraTarget );
    vec3 uu = normalize(cross( vec3(0.0,1.0,0.0), ww ));
    vec3 vv = normalize(cross(ww,uu));
    vec2 q = (gl_FragCoord.xy / iResolution.xy)  * invScale - offset;
    vec2 p = -1.15 + 2.0*q;
