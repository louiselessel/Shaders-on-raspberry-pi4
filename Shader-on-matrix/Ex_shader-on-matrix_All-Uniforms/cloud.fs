...
vec2 offset = vec2(invScale - 1.0) * 0.5;
    ...
    vec2 q = ( gl_FragCoord.xy / iResolution.xy) * invScale - offset; // normalization of sampling coordinates - you may have to delete offset if black output. And make sure the scaling is only added to the line that looks something like: vec2 name = gl_FragCoord/iResolution)
    vec2 p = -1.0 + 2.0*q;
